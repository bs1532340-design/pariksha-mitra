    ...
    <section className="relative z-10 py-12 md:py-20 px-4 md:px-6">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="text-center space-y-3 md:space-y-4 mb-12 md:mb-16">
          <h2 className="text-2xl md:text-3xl lg:text-4xl font-bold tracking-tight text-balance px-2">
            <span className="bg-gradient-to-r from-accent to-accent/60 bg-clip-text text-transparent">
              Simple, Transparent Pricing
            </span>
          </h2>
          <p className="text-sm md:text-lg text-muted-foreground max-w-2xl mx-auto text-balance px-2">
            ...
            <div
              ...
              className={`relative rounded-2xl border transition-all duration-500 hover:shadow-2xl ${
                plan.highlighted
                  ...
                  : 'border-border/50 bg-card/30 slide-up-delay-200 scale-up-hover'
              } backdrop-blur-sm p-6 md:p-8 flex flex-col group`}
            >
              {/* Highlight Badge */}
              {plan.highlighted && (
                <div className="absolute -top-3 md:-top-4 left-1/2 transform -translate-x-1/2 fade-in">
                  <span className="bg-accent text-accent-foreground px-3 md:px-4 py-1 rounded-full text-xs md:text-sm font-semibold whitespace-nowrap shadow-lg shadow-accent/30">
                    Most Popular
                  </span>
                </div>
              )}
              ...
              <button
                className={`w-full py-2.5 md:py-3 rounded-lg font-semibold transition-all duration-300 mb-6 md:mb-8 text-sm md:text-base scale-up-hover ${
                  plan.highlighted
                    ...
                    : 'bg-card/50 text-foreground border border-border/50 hover:bg-card/80 hover:border-accent/50'
                }`}
              >
                {plan.cta}
              </button>
              ...
              <div className="space-y-3 md:space-y-4 flex-1">
                {plan.features.map((feature, featureIndex) => (
                  <div key={featureIndex} className="flex items-start gap-2 md:gap-3 transition-smooth" style={{ animationDelay: `${featureIndex * 50}ms` }}>
                    <Check className="w-4 h-4 md:w-5 md:h-5 text-accent flex-shrink-0 mt-0.5" />
                    <span className="text-foreground text-xs md:text-sm group-hover:text-foreground/90 transition-colors duration-300">{feature}</span>
                  </div>
                ))}
              </div>
            </div>
        ...
        <div className="mt-12 md:mt-20 text-center space-y-2 md:space-y-3 px-4">
